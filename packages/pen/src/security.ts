export class PenSecurity {
   ...
   */
  public static validateXmlContent(xml: string, context: string = 'DOCX Part'): void {
  ...
  public static sanitizeHyperlink(url: string | undefined): string | undefined {
    ...
    if (trimmed.startsWith('#') || trimmed.startsWith('/')) {
      return trimmed; // internal document bookmarks
    }

    try {
      const parsed = new URL(trimmed);
      if (!this.ALLOWED_URL_PROTOCOLS.has(parsed.protocol)) {
        return undefined; // reject dangerous protocol
      }
      return trimmed;
    } catch {
      // If it starts with mailto: or safe scheme
